...
export default function CaseStudy({ project }: { project: PortfolioCase }) {
  return (
    <main id="main-content" className="site-shell case-study">
      ...
      {project.engineeringNotes?.length ? <section className="case-engineering" aria-labelledby="engineering-heading"><h2 id="engineering-heading">System and engineering notes</h2><div className="case-engineering-grid">{project.engineeringNotes.map((item, index) => <article key={item.title}><span className="case-number">{String(index + 1).padStart(2, "0")}</span><h3>{item.title}</h3><p>{item.body}</p></article>)}</div></section> : null}
